...
public class AbstractTriangleProcess {
    ...
    protected static void printUpdate(StopWatch stopWatch, int triangleIndex, Long triangleNumber, Integer[] factors) {
        if(factors.length > BIGGEST_FACTORS_SO_FAR) {
            stopWatch.suspend();
            System.out.print(String.format("%10d is triangle number #%6d with %4d factors: ", 
                    triangleNumber, triangleIndex, factors.length));
    ...
    protected static void printSolution(Long triangleNumber, int triangleIndex, int length, String time) {
        System.out.println("");
        System.out.println(triangleNumber + " is the first triangle number (the " + triangleIndex + "th) "
